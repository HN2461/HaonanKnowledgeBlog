...
- `src/styles/`：全局与通用样式。
- `public/notes/**`：Markdown 笔记内容；`public/notes-index.json` 为生成文件。
...
- `npm run generate:index`：笔记内容变更后重新生成分类与搜索索引。
...
- 若改动了 `public/notes/`，请运行 `npm run generate:index` 并提交 `public/notes-index.json`。
