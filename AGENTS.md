...
- 页面显示名称应统一：`frontmatter.title`、正文 H1、目录页入口文案、详情页标题应保持一致或至少语义完全一致。
- 如果文章属于“第X篇”系列，`title` 与正文 H1 默认保留“第X篇：”前缀；不要出现“文件名有第十篇、页面标题却没有第十篇”这类不一致情况。
- 当前站点详情页、列表页、搜索页优先读取 frontmatter 的 `title` 作为显示标题；如果页面标题显示不对，优先检查 frontmatter `title`、正文 H1 与文件名是否统一，而不是只改其中一处。
- frontmatter 模板：
```yaml
---
title: 示例标题
