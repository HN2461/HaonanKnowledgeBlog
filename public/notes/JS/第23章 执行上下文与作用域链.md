...
执行上下文与作用域链是JavaScript运行机制的核心：

### 核心要点

1. **执行上下文**：代码执行的环境，包含变量环境、词法环境和this绑定
2. **变量环境**：存储var声明和函数声明，支持提升
3. **词法环境**：存储let/const声明，有暂时性死区
4. **作用域链**：变量查找的路径，遵循词法作用域规则
5. **闭包机制**：内部函数对外部变量的持久引用
...
下一章我们将深入学习this绑定机制，理解JavaScript中最容易混淆的概念之一。
