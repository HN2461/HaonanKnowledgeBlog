---
title: uni-app WebSocket 与 STOMP 总览：分层设计与接入顺序
date: 2026-04-16
category: 项目复用技术
tags:
  - uni-app
  ...
  - 实时消息
  - 工具封装
description: 这是 WebSocket 专题的第 1 篇总览，系统梳理 uni-app 中 WebSocket 与 STOMP 为什么容易写乱，以及如何把协议常量、帧构建、连接管理、自动重连、消息解析、订阅恢复和页面消费方式拆成一套可直接复用的方案。
---

# uni-app WebSocket 与 STOMP 总览：分层设计与接入顺序

> 这是 `项目复用技术 / uni-app / WebSocket` 专题的第 1 篇。  
...
> 这篇文章的目标很明确：
>
> `把 uni-app 里的 WebSocket / STOMP 功能先搭出完整地图，后面再分篇逐步照着实现`
...
3. `底层客户端层`
   只负责 `uni.connectSocket` 的生命周期和 STOMP 协议发送接收
4. `连接管理层`
