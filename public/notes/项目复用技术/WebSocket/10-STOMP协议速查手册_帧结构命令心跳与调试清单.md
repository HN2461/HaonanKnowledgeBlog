...
| 类型 | 格式 | 含义 |
|------|------|------|
| Topic（广播） | `/topic/notifications` | 所有订阅者都会收到 |
| Queue（点对点） | `/queue/order-confirm` | 只有一个消费者收到 |
| User（用户专属） | `/user/queue/private` | 当前用户的私有队列 |
...
5. **destination 分类**：topic（广播）、queue（点对点）、user（用户专属）
