...
+ **GET 请求**：获取资源。
+ **POST 请求**：提交资源。
+ **JSON 返回值**：服务器返回 JSON 格式数据。<!-- 这是一张图片，ocr 内容为： -->
...
## 14、节流和防抖的目的
都是为了<u><font style="color:#DF2A3F;">限制函数的执行频次</font></u>，以优化函数触发频率过高导致的响应速度跟不上触发频率，防止在短时间内频繁触发同一事件而出现延迟，假死或卡顿的现象
...
## 45、Intl.DateTimeFormat 的 timeStyle 和 dateStyle 选项
简化日期时间格式化配置。例如：
...
## 50、Intl.DateTimeFormat 的 timeStyle 和 dateStyle 选项
简化日期时间格式化配置。例如：
