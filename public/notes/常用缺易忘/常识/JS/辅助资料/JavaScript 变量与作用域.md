...
## `**var**`** 声明的全局变量会挂载到 **`**window**`**，**`**let / const**`** 不会。**
### ✅ 核心原因（必答点）
+ `var` 是**函数作用域**，在全局声明时会同时成为 **全局对象（window）的属性**；
...
### 🔥 面试 3 句话标准口述版
**var 声明的全局变量会挂载到 window，而 let / const 不会。**  
**原因是 var 属于函数作用域，全局声明时会绑定到全局对象；而 let / const 是 ES6 的块级作用域设计，全局变量存放在独立的词法环境中，与 window 解耦。**  
**这样可以避免全局变量污染 window，是 ES6 对作用域模型的重要改进。**

如果你要，我也可以帮你把 **var / let / const + 提升 / TDZ / for 循环闭包** 一次性整理成一张「面试速查表」。
