...
> `2026-05-22` 校注：当前官方桌面 App 已明确支持 Windows-native 与 WSL agent 路线，所以这篇应理解为“某次本机环境异常复盘”，而不是“今天的 Codex 还必须依赖旧编辑器形态或旧调用链”。
> 定位：故障复盘篇，不是通用安装主线。  
> 适合谁看：主人已经遇到 Windows 终端、PowerShell、MCP、CC Switch、`node -e` 或命令调用异常时。  
> 先读哪篇：如果你还没建立基础心智，先读第三篇、第八篇；如果你已经在排障，再把第九篇一起对照看。  
> 小白读完目标：你应该能判断“这是仓库代码问题、Codex 终端环境问题，还是 PowerShell / 配置同步链路问题”，避免一上来就误删配置或重装一整套环境。

---

## 一、问题背景

使用 **Codex 桌面端、IDE 扩展或 Cursor 这类编辑器集成** 开发时，出现以下症状：
...
Windows PowerShell 5.1 并不等于“必然损坏”或“绝对不可用”，但它与 PowerShell 7 在模块兼容、编码、跨平台能力和部分现代工具链支持上确实存在差异。对 Codex 桌面端、IDE 扩展、本地脚本代理、MCP 启动命令这类链路来说，**同一台机器里混用两套 PowerShell，确实会让问题更难定位**。
...
1. 如果你在桌面 App 里切到了 WSL agent，记得重启 App
2. 如果你的 CLI 跑在 WSL，而桌面 App 跑在 Windows-native，默认不会自动共享同一个 `~/.codex`
3. 所以排障时要先分清：是 PowerShell 问题、是 `CODEX_HOME` 不一致，还是两者一起叠加了
...
- 如果涉及 Windows-native / WSL 双路线，也确认两边读取的是你预期的同一套 `CODEX_HOME` / `.codex`
