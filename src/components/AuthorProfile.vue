        ...
        <div class="profile-modal" v-if="showProfile" @click="closeProfile">
          ...
          <div class="profile-content">
            ...
            <div class="profile-tools">
              <button class="profile-tool-btn bookmarks-tool-btn" @click="openBookmarks">
                <div class="btn-icon">🔖</div>
                <div class="btn-text">
                  <span class="btn-title">书签收藏</span>
                  <span class="btn-subtitle">快速查找并打开你积累的网址入口</span>
                </div>
                <div class="btn-arrow">→</div>
              </button>

              <button class="profile-tool-btn relaxation-btn" @click="enterRelaxationMode">
                <div class="btn-icon">🌅</div>
                <div class="btn-text">
...
const loadProfileStats = async () => {
  try {
    const [noteData, bookmarkData] = await Promise.all([
      loadNotesIndex(),
      loadBookmarksIndex().catch(() => null)
    ])

    stats.value = {
      articles: noteData.totalNotes || noteData.allNotes?.length || 0,
      categories: noteData.totalCategories || noteData.categories?.length || 0,
      bookmarks: bookmarkData?.totalCount || 0,
      lastUpdated: formatUpdateDate(noteData.lastUpdated)
    }
  } catch (err) {
...
.profile-tool-btn:hover .btn-arrow {
