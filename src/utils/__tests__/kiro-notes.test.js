...
function getAllIndexedNotes(index) {
  if (Array.isArray(index?.allNotes)) {
  ...
  const flattened = (index?.categories || []).flatMap((category) => category.notes || [])
  const uniqueByPath = new Map()

  flattened.forEach((note) => {
    if (note?.path) {
      uniqueByPath.set(note.path, note)
    }
  })
  ...
  it('Property 2: frontmatter 字段值合法性', () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...noteFiles), (file) => {
        ...
        const descLen = countVisibleChars(fm.description)
        const descValid = descLen >= 60 && descLen <= 160
  ...
  it('Property 4: 正文结构完整性', () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...noteFiles), (file) => {
        ...
        const bodyWithoutCodeBlocks = stripFrontmatterAndCodeBlocks(fs.readFileSync(file, 'utf-8'))
        const h1Count = (bodyWithoutCodeBlocks.match(/^# .+/gm) || []).length
        const hasToc = body.includes('[[toc]]')
        const hasCaveats = /#{2,3} (注意事项|常见问题)/.test(bodyWithoutCodeBlocks)
