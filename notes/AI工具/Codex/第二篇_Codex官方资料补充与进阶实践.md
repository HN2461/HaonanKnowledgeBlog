...
- `--full-auto`：这是旧资料里常见的兼容别名思路，当前更建议直接显式写 `--sandbox workspace-write --ask-for-approval on-request`
...
- 登录缓存可能落在 `~/.codex/auth.json`，也可能落在系统凭据库（keyring）
- `~/.codex/auth.json` 一旦存在，就应按密码文件对待，不要提交到仓库
