...
tags:
  ...
  - AI规则文档
  - 上下文管理
description: 从"AI 每次都忘了你的项目规范"这个痛点出发，解释规则文档的本质、解决的核心问题，并横向对比 Claude Code、Codex、Cursor、Windsurf、Kiro、Gemini CLI、GitHub Copilot 七大工具的规则文件名、存放位置与加载方式。
---

# 第一篇：AI 工具规则文档是什么，为什么需要它

> 资料来源：Anthropic 官方文档、OpenAI agents.md GitHub 仓库、vibecoding.app、各工具官方文档，整理时间：2026-04。
...
| Cursor | `.cursor/rules/*.mdc` | `.cursor/rules/` 目录 | 全局规则设置 |
| Windsurf | `.windsurf/rules/*.md` | `.windsurf/rules/` 目录 | `~/.codeium/windsurf/memories/global_rules.md` |
| Kiro | Steering 文件 | `.kiro/steering/*.md` | 用户级 Steering |
| Gemini CLI | `GEMINI.md` | 项目根目录 | `~/.gemini/GEMINI.md` |
| GitHub Copilot | `copilot-instructions.md` | `.github/` 目录 | 无 |

### 跨工具标准：AGENTS.md

2025 年 8 月，OpenAI 发布 `AGENTS.md` 格式，随后被 Cursor、Windsurf、GitHub Copilot、Amp、Devin 等工具相继采纳。2025 年 12 月，该格式被移交给 **Linux 基金会旗下的 Agentic AI Foundation（AAIF）**，成为 AI 编程生态的开放标准。

目前支持直接读取 `AGENTS.md` 的工具：

| 工具 | 支持情况 |
|------|---------|
| OpenAI Codex CLI | ✅ 原生支持（发起者） |
| GitHub Copilot | ✅ 2025-08 起支持 |
| Cursor | ✅ 支持 |
| Windsurf | ✅ 支持（根目录=always_on，子目录=glob） |
| Kiro | ✅ 原生支持（可放工作区根目录或 `~/.kiro/steering/`） |
| Amp（Sourcegraph） | ✅ 支持，无 AGENTS.md 时回退到 CLAUDE.md |
| Devin | ✅ 支持 |
| Aider | ✅ 支持 |
