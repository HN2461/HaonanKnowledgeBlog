...
tags:
  ...
  - 终端界面
  - 排错
description: 基于主人整理的 Codex CLI 终端界面线索，用文字图逐屏解释首页、模型选择、推理强度、权限、状态页、MCP、插件、skills、experimental 与错误提示，帮助把英文终端界面读成可操作的开发判断。
---
...
| `/experimental` | 哪些实验能力开了，出了问题能不能回滚 |
...
## 10. `/experimental`：新能力先理解，再启用

`experimental` 的关键词不是“高级”，而是“可能变化”。

```mermaid
flowchart TD
  A["/experimental"] --> B["实验能力列表"]
  B --> C["解决什么具体问题？"]
...
`/plugins`、`/skills`、`/experimental` 这类入口的具体显示会跟 CLI 版本、插件安装状态和账号能力有关。最稳的核对方式不是记某个静态截图，而是在当前 Codex CLI 里用 `/`、`/status`、`/plugins`、`/skills`、`/mcp` 反查实际可用项。
