...
| OPENAI_API_KEY | API Key 字符串 | sk-*** | 作为身份凭据参与请求 |
...
### 1.5 与其他文件的关系
如果你的 provider 还在直接写 `experimental_bearer_token`，就可能与 `auth.json` 或 keyring 中的认证状态同时存在。  
因此改 key 时，除了检查 `auth.json`，也要一起检查 provider 配置里是否残留旧凭据。
...
3. 真正要写入自己当前环境的默认模型，请先看第三篇、第五篇和第十二篇里已经校准过的最新口径
