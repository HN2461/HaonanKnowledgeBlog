...
tags:
  ...
  - emit
  - provide/inject
description: 单向数据流：谁的数据谁修改，子组件不能直接修改父组件传入的props。耦合度最小化：优先使用官方推荐方案，避免强耦合方式（如$parent/$children）。场景决定方案：不同层级、不同规模的项目选择最适合的通信方式。
---
...
## ⚙️ 核心原理
+ 父组件通过属性形式传递数据，**<font style="color:rgba(0, 0, 0, 0.95);">Props 绑定语法</font>**<font style="color:rgba(0, 0, 0, 0.95);">：</font>`<font style="color:rgb(0, 0, 0);">:属性名="数据"</font>`<font style="color:rgba(0, 0, 0, 0.95);"> 是 </font>`<font style="color:rgb(0, 0, 0);">v-bind:属性名="数据"</font>`<font style="color:rgba(0, 0, 0, 0.95);"> 的简写，用于动态传递父组件的响应式数据或函数；</font>
...
// ✅ 保持响应式
// 把一个响应式对象转换成普通对象，对象的每个属性都是一个 ref 引用，保持原对象的响应式。
const { title } = toRefs(props)
// 为响应式对象（如 props）的单个属性创建一个 ref 引用，适用于只需要解构单个属性的场景。
...
### <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">核心差异总结</font>
| **<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">特性</font>** | **<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">Vue 2 (Options API)</font>** | **<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">Vue 3 (Composition API)</font>** |
| :--- | :--- | :--- |
| <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">声明方式</font> | `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">props: { ... }</font>` | `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">defineProps({ ... })</font>` |
| <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">JS 中访问方式</font> | `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">this.propsName</font>` | `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">props.propsName</font>`<br/><font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">（无 this）</font> |
| <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">引用类型默认值</font> | <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">函数返回（如 </font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">default(){}</font>`<br/><font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">）</font> | <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">同 Vue 2</font> |
| <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">兼容性</font> | <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">仅选项式 API</font> | <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">同时支持选项式 / 组合式 API</font> |
...
### 父组件
`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">v-model:prop名</font>` 语法糖简化 `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">:prop名 + @update:prop名</font>` 的写法  ；`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">update:propName</font>` 是 Vue 约定的事件命名规范，用于子组件通知父组件更新指定 prop，实现 “伪双向绑定”（本质仍是单向数据流）。  
...
## 🎯 适用场景
**Vue2 专用**，无嵌套关系的兄弟组件/任意组件通信（轻量级，Vue3 推荐用 `mitt` 或 Pinia）
...
默认情况下，`$attrs` 中的<font style="color:rgba(0, 0, 0, 0.95);">未被声明为 </font>`<font style="color:rgb(0, 0, 0);">props</font>` 属性会渲染到组件的**根元素**上。即可以F12审查到。
...
**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">关键注意点</font>**：

+ `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">$refs</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);"> 是</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">渲染完成后</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">才生成的，在 </font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">created</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);"> 钩子中无法访问（此时 DOM 未渲染），需在 </font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">mounted</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);"> 中使用；</font>
+ `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">ref</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);"> 是 “单向绑定”，修改 </font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">$refs</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);"> 不会触发视图更新，仅用于</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">读取 / 操作</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">，不建议依赖它做响应式逻辑。</font>
...
## ⚠️ $parent（不推荐）
`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">$parent</font>` 是组件实例的内置属性，用于**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">直接访问当前组件的父组件实例</font>**，可以读取父组件的 data、调用父组件的方法。  
...
## ❌ $children（Vue3 已废弃）
`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">$children</font>` 是父组件实例的内置属性，返回**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">当前父组件所有直接子组件的实例数组</font>**，可以遍历访问子组件的属性 / 方法。  
