...
## 一、Vue 2.x 的响应式原理
在 Vue 2.x 中，响应式系统主要依赖于 `Object.defineProperty()` 方法，通过拦截对象属性的读取（`get`）和修改（`set`）操作来实现数据劫持。对于数组类型，则通过重写数组的变更方法（如 `push`、`pop` 等）来实现拦截。

然而，这种方式存在一些局限性：

+ **新增属性、删除属性**：界面不会更新。因为 `Object.defineProperty()` 只能对已存在的属性进行拦截，对于新增或删除的属性，无法自动触发更新。
+ **直接通过下标修改数组**：界面不会自动更新。由于数组的下标修改不会触发重写的方法，因此需要借助 `Vue.set` 或 `this.$set` 等方法来实现响应式更新。

## 二、Vue 3.0 的响应式原理
Vue 3.0 对响应式系统进行了重构，引入了 ES6 的 `Proxy` 和 `Reflect` API，解决了 Vue 2.x 中的许多问题。

+ **Proxy**：代理整个对象，拦截对象的**所有操作**（读取、设置、新增、删除属性等）。
+ **Reflect**：与 Proxy 配对的反射 API，用于对被代理对象执行**实际操作**，保证行为符合原生语义。

### 1. Proxy（代理）
`Proxy` 可以理解为一个中间层，它能够拦截并自定义对象上各种操作的行为，比如属性的读取、设置、删除等。在 Vue 3.0 中，`Proxy` 被用来拦截对象中任意属性的变化，包括属性值的读写、属性的添加、属性的删除等。
...
### 2. Reflect（反射）
`Reflect` 是一个内置对象，它提供了对对象操作的反射方法，比如 `Reflect.get()`、`Reflect.set()` 等。这些方法与 `Proxy` 的拦截器方法相对应，用于对被代理对象的属性进行实际操作。
...
通过 `Proxy` 和 `Reflect` 的结合使用，Vue 3.0 实现了一个更强大、更灵活的响应式系统。

## 三、ref 和 reactive 的响应式原理
### 1. ref 函数
`ref` 主要用于定义基本数据类型的响应式数据。它通过**类（class）的 getter/setter 访问器属性**实现响应式。当 `ref` 包装对象类型时，内部会调用 `reactive` 转换为 Proxy。
...
### 2. reactive 函数
`reactive` 用于定义对象类型的响应式数据。它基于 ES6 的 `Proxy` 实现，通过代理对象操作源对象内部数据，实现深层次的响应式。Vue3 采用**懒代理（lazy reactive）**策略：只有在访问嵌套对象时才会递归代理，性能优于 Vue2 的初始化递归劫持。
...
state.info.name = 'Vue 3'; // 深层次修改也会触发响应式更新（info 在首次访问时被代理）
```
...
**边界情况**：Vue3 中 `reactive` 返回的是**代理对象**，与原始对象 `!==`，需始终使用代理对象操作数据。

---

## 四、总结
+ Vue2 核心：`Object.defineProperty` → 逐个属性劫持 → 有局限性，需特殊 API 补救。
+ Vue3 核心：`Proxy` + `Reflect` → 全对象代理 → 无监听盲区，性能更优，API 更简洁。
...
**Reflect** 是 **JavaScript 官方（ECMA 国际）在 ES6 标准里新增的内置对象**，**Proxy（代理对象）和 Reflect 一样，也是 JavaScript 官方 ES6 标准里的原生特性，** 和 Vue 没有半毛钱关系 ——Vue3 只是 "借用" 了这个原生 API，让自己的响应式系统更规范、更稳定而已。
...
const 代理对象 = new Proxy(原始对象, {
  ...
  // Proxy 共支持 13 种拦截操作，Vue3 主要使用上述 3 种
});
```
...
### 为什么 Vue2 替换整个对象反而能生效？
对比一下更易理解：

+ **Vue2**：响应式是劫持 `data` 里的**属性**（`Object.defineProperty`），如果替换整个 `this.user = { ... }`，本质是修改 `this` 上的 `user` 属性（这个属性早已被劫持），所以能触发更新；
