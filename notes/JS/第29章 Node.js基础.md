...
Node.js为JavaScript提供了服务端运行环境：

### 核心要点
1. **运行时环境**：V8引擎 + libuv + 内置模块
2. **模块系统**：CommonJS规范，支持ES模块
3. **核心模块**：fs文件系统、path路径处理、http网络通信
4. **异步机制**：事件循环、非阻塞I/O、事件驱动
