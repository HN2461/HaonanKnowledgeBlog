...
export const dailyChangeSummary = {
  ...
  items: [
    ...
    },
    {
      ...
      content: '第一点：将 public/notes/常用缺易忘/常识/Vue/vue辅助/Router.md 再次重写为《Vue Router 超详细总笔记》，从“复习优先的项目版”进一步扩成“一站式知识手册”；第二点：围绕官方资料补全知识体系，系统整理 route/routes/router、router-link、router-view、命名路由、重定向、别名、404、命名视图、嵌套路由、路由规则对象字段等基础与进阶内容；第三点：重点把路由传参完整展开，细分 query、params、props 三大类，并把字符串拼接、对象写法、name/path 区别、多参数、hash、编程式跳转、接收方式和高频注意点全部补齐；第四点：补充动态路径匹配语法，包括自定义正则、可选参数、可重复参数，并区分“动态参数路由”和 `addRoute()` 这种运行时动态添加路由；第五点：补齐守卫分类、执行顺序、scrollBehavior、懒加载、keep-alive、navigation failures、Vue2/Vue3 差异、高频问答与踩坑清单，强化长期复习可用性；第六点：重新生成 notes-index.json 与 search-index.json，当前总笔记数 327 篇。',
    },
  ],
}
